...
function parseQueryString() {
    ...
    if (!num_rows) num_rows = "10";
    if (!page) page = "1";
    if (!filter_by_player_name) filter_by_player_name = "all";
    return { num_rows, page, filter_by_player_name };
}
...
function processPrevButton() {
    var url = new URL(window.location.href);
    var currentPageNum = parseInt(url.searchParams.get("page"));
    var nextPageNum = currentPageNum > 1 ? currentPageNum - 1 : currentPageNum;
    url.searchParams.set("page", nextPageNum);
    window.location.href = url;
}
